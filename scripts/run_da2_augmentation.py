...
# Metodi stain normalization: richiedono un'immagine target
...
def run_processing(config):
    ...
    np.random.seed(random_seed)                     # riproducibilità del dataset generato
    ...
    for cur_dir in config['datasets']['folders']:
        ...
        # pool target per le stain norm: prime N immagini 
        ...
        for filename in file_names:
            ...
            for method_name in method_list:
                if not hasattr(da2_methods, method_name):
                    print(f"Metodo '{method_name}' non in da2_methods.py — salto")
                    continue
                aug_function = getattr(da2_methods, method_name)

                try:
                    if method_name in STAIN_METHODS:
                        # target random dal pool (replica j=round(1+rand*89))
                    ...
                    # filtro foreground (rilevante per le geometriche che spostano la mask)
                    ...
                    print(f"[{method_name}] fallito su {filename}: {e} — salto")
                    continue
